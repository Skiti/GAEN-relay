 ...
 * com.spritz.gaenrelay.bluetooth.AdvertiseDataGenerator and the                    *
 * com.spritz.gaenrelay.bluetooth.AdvertiseDataParser interfaces. You may           *
 * copy and distribute such a system following the terms of the GNU GPL for             *
 * Beacon Simulator and the licenses of the other code concerned, provided that         *
 * you include the source code of that other code when and as the GNU GPL               *
 * requires distribution of source code and provided that you do not modify the         *
 * com.spritz.gaenrelay.bluetooth.AdvertiseDataGenerator and the                    *
 * com.spritz.gaenrelay.bluetooth.AdvertiseDataParser interfaces.                   *
 *                                                                                      *
...
import com.spritz.gaenrelay.bluetooth.BtNumbers;
import com.spritz.gaenrelay.bluetooth.ByteTools;
import com.spritz.gaenrelay.bluetooth.IBeaconParser;
import com.spritz.gaenrelay.bluetooth.model.AltBeacon;
import com.spritz.gaenrelay.bluetooth.model.ImmuniBeacon;
import com.spritz.gaenrelay.bluetooth.model.BeaconModel;
import com.spritz.gaenrelay.bluetooth.model.BeaconType;
import com.spritz.gaenrelay.bluetooth.model.IBeacon;
import com.spritz.gaenrelay.component.DialogCopyBeacon;
import com.spritz.gaenrelay.component.ViewEditAltBeacon;
import com.spritz.gaenrelay.component.ViewEditEddystoneTlm;
import com.spritz.gaenrelay.component.ViewEditEddystoneUid;
import com.spritz.gaenrelay.component.ViewEditEddystoneUrl;
import com.spritz.gaenrelay.component.ViewEditIBeacon;
import com.spritz.gaenrelay.component.ViewEditImmuniBeacon;
...
public class FragmentDetailedScan extends Fragment {
    ...
    public static final String SCAN_RESULT = "com.spritz.gaenrelay.SCAN_RESULT";
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        boolean canCopy = ! (BeaconType.raw.equals(mBeaconModel.getType()));
        if ( canCopy ) {
    ...
    private void generateCards(ViewGroup cardContainer) {
        ...
        for (ADStructure structure : mAdStructures) {
            sLogger.debug("structure being evaluated");
            if (structure instanceof EddystoneUID) {
                ...
                sLogger.debug("structure is UUIDs");
                fillUuidCard((UUIDs)structure, cardContainer);
                ImmuniBeacon immuniBeacon;
                for ( UUID uuid : ((UUIDs) structure).getUUIDs() ) {
                    sLogger.debug("UUID:");
                    sLogger.debug(String.valueOf(uuid));
                    if (uuid.equals(UUID.fromString("0000FD6F-0000-1000-8000-00805F9B34FB"))) {
                        sLogger.debug("found ImmuniBeacon");
                        immuniBeacon = ImmuniBeacon.parseRecord(mScanResult.getScanRecord());
                        if (immuniBeacon != null) {
                            mBeaconModel = new BeaconModel(BeaconType.immunibeacon);
                            mBeaconModel.setImmuniBeacon(immuniBeacon);
                            mBeaconModel.setBleMacAddress(mScanResult.getDevice().getAddress());
                            fillImmuniBeaconCard(mBeaconModel, cardContainer);
                            new ActivitySaveRelay().execute(mBeaconModel);
                        }
                    }
                }
            }
            else if (structure instanceof ServiceData) {
                sLogger.debug("structure is ServiceData");
                fillServiceDataCard((ServiceData)structure, cardContainer);
            }
            else if (structure instanceof Flags) {
                sLogger.debug("structure is Flags");
    ...
    private void fillImmuniBeaconCard(BeaconModel model, ViewGroup cardContainer) {
        sLogger.debug("fillImmuniBeaconCard");
        ViewEditImmuniBeacon immuniBeaconView = new ViewEditImmuniBeacon(getContext());
        immuniBeaconView.loadModelFrom(model);
        immuniBeaconView.setEditMode(false);
        cardContainer.addView(immuniBeaconView);
    }
